/**
 * Database service for encrypted local storage
 * Handles all SQLite operations with encryption
 */
...
export class DatabaseService {
   ...
   */
  public async searchMessages(query: string, limit: number = 100): Promise<ChatMessage[]> {
    if (!this.db) throw new Error('Database not initialized');
    
    // Note: This is a simplified search. In production, we'd need to decrypt
    // and search, which is computationally expensive. Consider using FTS.
    const result = await this.db.select(`
      SELECT * FROM messages 
    ...
    // Filter results by decrypting and searching content
    const filteredResults: ChatMessage[] = [];
    for (const row of result) {
      try {
        const decryptedContent = this.crypto.decrypt(row.content as string);
